...
class Robot extends EventEmitter {
  constructor({ accelerometerPollInterval, rangeFinderPollInterval }) {
    super()
    bindAll(Object.getOwnPropertyNames(Robot.prototype), this)
    if (!isNumber(accelerometerPollInterval)) throw new Error('Missing required parameter: accelerometerPollInterval')
    if (!isNumber(rangeFinderPollInterval)) throw new Error('Missing required parameter: rangeFinderPollInterval')

    this.leftMotor = new Motor({ forwardPin: 18, backwardPin: 23 })
    this.rightMotor = new Motor({ forwardPin: 17, backwardPin: 22 })
    this.rangeFinder = new RangeFinder({ echoPin: 4, pollInterval: rangeFinderPollInterval, triggerPin: 27 })
    this.rangeFinder.on('distance', (distance) => {
      this.emit('distance', distance)
      debug('distance', distance)
    })
    this.accelerometer = new Accelerometer({ address: 0x1D, pollInterval: accelerometerPollInterval })
